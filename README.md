...
**Описание основных настроек в этом cloud-init YAML:**

- `users`:  
  Создаётся пользователь с именем `ubuntu`.  
  Он добавлен в группу `sudo` для получения прав администратора без необходимости ввода пароля (`sudo: ["ALL=(ALL) NOPASSWD:ALL"]`).  
  Для этого пользователя настраивается оболочка `/bin/bash`.  
  Также указывается публичный SSH-ключ `ssh_authorized_keys`, полученный из переменной `${ssh_public_key}`, который будет добавлен в файл `~ubuntu/.ssh/authorized_keys`, чтобы обеспечить доступ по SSH без пароля.

- `package_update: true`:  
  При запуске ВМ будет выполнено обновление локального кэша пакетов (`apt update` или аналог), чтобы получить актуальную информацию о доступных пакетах.

- `package_upgrade: false`:  
  Обновление всех установленных пакетов до новых версий (через `apt upgrade`) не производится — таким образом сохраняется текущая версия ОС и пакетов.

- `packages`:  
  Список пакетов, которые будет пытаться установить system package manager (apt). Здесь указываются `docker` и `docker-compose`. Это означает, что сразу после создания ВМ на неё будет установлен Docker и Docker Compose, необходимые для работы с контейнерами.

Таким образом, данный cloud-init скрипт выполняет базовую настройку ВМ:

- Создаёт пользователя с удобным доступом по SSH через публичный ключ.  
- Обновляет индекс пакетов, но не обновляет сами пакеты.  
- Устанавливает Docker и Docker Compose для работы с контейнерами.

Этот подход широко применяется для автоматической подготовки виртуальных машин под нужды приложений и сервисов без необходимости ручной настройки после запуска.
